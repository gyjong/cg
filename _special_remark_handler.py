...
def create_or_load_vectorstore(sources: List[str], index_name: str = "index_remark") -> FAISS:
...
sources = [
    "./index_remark/cherry_rmk_guidelines.pdf"
]
...
rag_prompt = PromptTemplate(
    ...
    3. Use bullet points to organize information when appropriate.

    4. If the question is unclear, ask the user for clarification.

    5. For sanctions-related questions:
    • Reference relevant regulations, laws, and case law
    • Include applicable sanction provisions
    • Explain potential penalties
    • Describe enforcement mechanisms

    6. Respond in Korean if the question is in Korean.

    7. Respond in French if the question is in French.

    8. If you don't know the answer, simply state "I don't know" without attempting to fabricate a response.
...
def run():
    ...
    # Chat input
    prompt = st.chat_input("Any question about Booking Remark?")
